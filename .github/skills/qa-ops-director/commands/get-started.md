...
Display a welcome message and check basic prerequisites:

```
👋 Welcome to QA Ops Director!

I'll walk you through setup step by step. At each step I'll verify
that everything works before moving on.
...
| Claude Code | Check if running in Claude Code context | Yes |
...
**3a. Staging login credentials (REQUIRED)**
...
**3b. Atlassian API Token (REQUIRED for Jira/Confluence)**
...
**3c. TestRail API Key (REQUIRED for test management)**
...
**3d. Jira Keychain (REQUIRED for bug reports)**
...
**3e. MCP Server Config (auto-created if needed)**

Check if `~/.claude/.mcp.json` exists:
- If yes → verify it has `atlassian` server
...
| **Jira REST** | Check keychain | `security find-generic-password -s jira-api-token -w` |
| **Jira MCP** | Search 1 issue | `mcp_atlassian_search_jira_issues(jql="project=AE", limit=1)` |
| **Confluence MCP** | List spaces | `mcp_atlassian_get_confluence_spaces(keys=["EP"])` |
...
| Figma MCP | ⏭️ Optional (authenticate via /mcp if needed) |
...
| MCP not found | Guide creating `~/.claude/.mcp.json` |
| Test failures on smoke | Offer `/auto-triage` to analyze |
| User on Windows/Linux | Adjust Keychain command to use env vars instead |

## Output

Single interactive wizard session. No files created (credentials go into existing .env files).
Final output is the readiness checklist showing all green checks.
